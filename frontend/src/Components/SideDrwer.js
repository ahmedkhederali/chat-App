...
function SideDrwer() {
  ...
  const { isOpen, onOpen, onClose } = useDisclosure(); // Used To open Search Drawer
  ...
  const accessChat = async (userId) => {
    try {
      ...
      //Check before Create Chat if it exist or not
      if (!chats.find((c) => c._id === data._id)) setChats([data, ...chats]);

      setSelectedChat(data);
      setLoadingChat(false);

      onClose(); //to close seacrh After make chat
    } catch (error) {
      toast({
        ...
        <div>
          <Menu>
            ...
            <div style={{    
              ...
              position: "absolute",
              top: "9px",
              right: "108px"}}>{notification.length}</div>
